...
leveldb прекрасно зарекомендовал себя во многих сервисах мамбы. Однако, со временем, хранилища стали разрастаться
и стало тяжело их администрировать, т.к. не понятно где какая, важная или не очень, информация находиться. Достаточно 
проблематично выявить сервис, который из-за некорректной реализации приводит к разбуханию хранилища.

key-value хранилища по своей природе имеют полскую структуру, и, чтобы как-то кластеризовать огромное число 
ключей используют префиксы, по сути пространства имен, которые являются частью каждого ключа. Но, как правило, значения
ключа число, небольшая строка или объект, т.е. основной объем хранимых данных приходится на ключи, каждый из которых 
дублирует довольно массивный префикс.
